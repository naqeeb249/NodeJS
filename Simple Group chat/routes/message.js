...
router.get('/',(req,res) => {
  /*   fs.readFile('message.txt', (err,file) => {
    ...
    fs.readFile('message.txt',(err,data) => {
        ...
        console.log(data);
        res.send(`<div>${data != null ? data:"No Message Sent"}<form  action = "/message"  method = "POST"><input id = "send" type = "text" name="title"><button onclick ="document.getElementById('send').value += '-' + window.localStorage.getItem('username')" type ="submit">Send</button></form></div>`);
    })
})
